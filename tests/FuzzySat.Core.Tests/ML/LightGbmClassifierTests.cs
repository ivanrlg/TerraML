...
public class LightGbmClassifierTests
{
    [Fact]
    public void Train_ClassifiesCorrectly()
    {
        ...
        var classifier = LightGbmClassifier.Train(samples, extractor,
            numberOfLeaves: 15, numberOfIterations: 50, learningRate: 0.1);
    ...
    public void Train_EmptySamples_Throws()
    {
        ...
        var act = () => LightGbmClassifier.Train(samples, extractor);

        act.Should().Throw<ArgumentException>();
    }

    [Fact]
    public void Train_InvalidLeaves_Throws()
    {
        var (extractor, samples) = MakeTrainingData();

        var act = () => LightGbmClassifier.Train(samples, extractor, numberOfLeaves: 1);

        act.Should().Throw<ArgumentOutOfRangeException>();
    }

    [Fact]
    public void Train_InvalidLearningRate_Throws()
    {
        var (extractor, samples) = MakeTrainingData();

        var act = () => LightGbmClassifier.Train(samples, extractor, learningRate: 0);
